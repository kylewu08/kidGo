 ...
 *
 * ⚠️ 設計架構書 §2 把「午睡起點前無法返家」列為 L1 硬性過濾（剔除），
 * 但 §6.2 的 Stage 1 正式清單裡沒有這一條，而 §6.3 把作息契合度放在 Stage 2 佔 30%。
 * 此處依 §6.2 實作為**評分**而非剔除：撞到午睡的地點會被大幅扣分，但不會消失。
 * 這個歧異已回報，若決定改為硬性剔除，規則要搬到 filters.ts。
 */
